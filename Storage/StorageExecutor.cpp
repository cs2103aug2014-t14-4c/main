...
State StorageExecutor::loadFromStorage(){
	State stateToLoad; vector<string> storageToConvert; vector<Task> myConvertedTask; 
	...
	vector<string>::iterator myStorageIterator = storageToConvert.begin();
...
void StorageExecutor::saveToStorage(State stateToSave){
	...
	StorageDatabase myStorageDatabase;
	StorageConverter myStorageConverter; 
	vector<string> convertedTaskStringStorage; 
	string individualConvertedTask;
